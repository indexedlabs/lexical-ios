...
- For screenshots or external files referenced in spec/task text:
  - Upload: `mt file upload <path>`
  - Embed in rich text with returned `file_id`: `![[file <file_id>]]` (block) or `[[file <file_id>]]` (inline)
  - Optionally link artifact evidence: `mt link --from <spec-or-task> --rel documented_by --to-type file --to-ref mighty-file:<file_id>`
