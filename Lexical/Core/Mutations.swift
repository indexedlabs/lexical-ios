...
@MainActor
internal func handleTextMutation(textStorage: TextStorage, rangeOfChange: NSRange, lengthDelta: Int)
  throws
{
  ...
  if newRange.location + newRange.length <= textStorage.length {
    ...
    throw LexicalError.invariantViolation(
      "mutation out of bounds: possible incorrect usage of non-controlled mode")
  }

  editor.log(
    .other, .verbose, "Before updating range cache: dirty nodes count \(editor.dirtyNodes.count)")

  updateRangeCacheForTextChange(nodeKey: nodeKey, delta: lengthDelta)

  editor.log(
    .other, .verbose, "After updating range cache: dirty nodes count \(editor.dirtyNodes.count)")
  ...
  let styles = AttributeUtils.attributedStringStyles(
    from: node, state: editorState, theme: editor.getTheme())
