...
open class ListPlugin: Plugin {
    ...
    do {
      ...
      _ = editor.registerCommand(
        ...
        listener: { [weak editor] payload in
          ...
          try? insertList(
            editor: editor, listType: .bullet, withPlaceholders: self.withPlaceholders)
          return true
        })
      ...
      _ = editor.registerCommand(
        ...
        listener: { [weak editor] payload in
          guard let editor else { return false }
          try? insertList(
            editor: editor, listType: .number, withPlaceholders: self.withPlaceholders)
          return true
        })
      ...
      try editor.registerCustomDrawing(
        ...
        guard let attributeValue = attributeValue as? ListItemAttribute,
          let textStorage = layoutManager.textStorage as? TextStorage
        else {
          return
        }
  ...
  public func hitTest(
    at point: CGPoint, lineFragmentRect: CGRect, firstCharacterRect: CGRect,
    attributes: [NSAttributedString.Key: Any]
  ) -> Bool {
    guard let listItemAttribute = attributes[.listItem] as? ListItemAttribute,
      listItemAttribute.listType == .check
    else {
      return false
    }
    ...
    do {
      try editor?.update {
        guard let editorState = getActiveEditorState(),
          let node = editorState.getNodeMap()[listItemAttribute.itemNodeKey] as? ListItemNode
        else {
          return
        }
